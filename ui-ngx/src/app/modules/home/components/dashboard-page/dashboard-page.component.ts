...
import {
  ...
  FiltersDialogData,
} from "@home/components/filter/filters-dialog.component";
    ...
    OnDestroy
{
  ...
  get hideToolbar(): boolean {
    return (
      ((this.hideToolbarValue || this.hideToolbarSetting()) && !this.isEdit) ||
      this.isEditingWidget ||
      this.isAddingWidget
    );
  }
  ...
  rightLayoutSize: { width: string; height: string } = {
    width: "100%",
    height: "100%",
  };

  private dashboardLogoCache: SafeUrl;
  private defaultDashboardLogo = "assets/logo_dashboard.svg";
  ...
  get mobileDisplayRightLayoutFirst(): boolean {
    return (
      this.isMobile &&
      this.layouts.right.layoutCtx.gridSettings?.mobileDisplayLayoutFirst
    );
  }
  ...
  @ViewChild("dashboardWidgetSelect")
  ...
  ngOnInit() {
    this.rxSubscriptions.push(
      this.route.data.subscribe((data) => {
        ...
        if (this.embedded) {
          dashboardPageInitData = {
            dashboard: this.dashboardUtils.validateAndUpdateDashboard(
              this.dashboard
            ),
      ...
      this.rxSubscriptions.push(
        this.route.queryParamMap.subscribe((paramMap) => {
          if (paramMap.has("reload")) {
            ...
            setTimeout(() => {
              this.mobileService.handleDashboardStateName(
                this.dashboardCtx.stateController.getCurrentStateName()
              );
              this.mobileService.onDashboardLoaded(
                this.layouts.right.show,
    ...
    this.rxSubscriptions.push(
      this.changeMobileSize.pipe(distinctUntilChanged()).subscribe((state) => {
        this.isMobile = state;
      ...
      this.breakpointObserver
        ...
        .pipe(
          ...
          distinctUntilChanged((_, next) => {
            ...
            return (
              this.layouts.main.layoutCtx.breakpoint ===
              nextBreakpointConfiguration
            );
          }),
          skip(1)
        )
        .subscribe(() => {
          this.dashboardUtils.updatedLayoutForBreakpoint(
            this.layouts.main,
            this.dashboardCtx.breakpoint
          );
          this.updateLayoutSizes();
        })
    );
  ...
  ngAfterViewInit() {
    ...
    if (
      !this.widgetEditMode &&
      !this.readonly &&
      this.dashboardUtils.isEmptyDashboard(this.dashboard)
    ) {
  ...
  private init(data: DashboardPageInitData) {
    ...
    if (this.route.snapshot.queryParamMap.has("hideToolbar")) {
      this.hideToolbar =
        this.route.snapshot.queryParamMap.get("hideToolbar") === "true";
    }
    ...
    this.dashboardCtx.dashboardTimewindow =
      this.dashboardConfiguration.timewindow;
    this.layouts.main.layoutCtx.widgets = new LayoutWidgetsArray(
      this.dashboardCtx
    );
    this.layouts.right.layoutCtx.widgets = new LayoutWidgetsArray(
      this.dashboardCtx
    );
    ...
    this.dashboardCtx.aliasController = this.parentAliasController
      ? this.parentAliasController
      : new AliasController(
          this.utils,
          this.entityService,
          this.translate,
  ...
  public showCloseToolbar() {
    return (
      !this.toolbarAlwaysOpen() && !this.isEdit && !this.showRightLayoutSwitch()
    );
  }
  ...
  public toolbarAlwaysOpen(): boolean {
    if (
      this.dashboard.configuration.settings &&
  ...
  private getTranslatedDashboardTitle(): string {
    return this.utils.customTranslation(
  ...
  public showDashboardLogo(): boolean {
    if (
      ...
      return (
        this.dashboard.configuration.settings.showDashboardLogo &&
        (this.forceFullscreen || this.singlePageMode || this.isFullscreen)
      );
    } else {
      return false;
    }
  ...
  public get dashboardLogo(): string {
    return (
      this.dashboard.configuration.settings.dashboardLogoUrl ||
      this.defaultDashboardLogo
    );
  }
  ...
  private updateMainLayoutSize(): boolean {
    ...
    if (this.isEditingWidget && this.editingLayoutCtx.id === "main") {
      ...
      this.mainLayoutSize.width =
        this.layouts.right.show && !this.isMobile
          ? this.calculateWidth("main")
          : "100%";
    }
    if (!this.isEditingWidget || this.editingLayoutCtx.id === "main") {
    ...
    if (this.isEdit && !this.isEditingWidget) {
      const xOffset =
        this.dashboardContainer.nativeElement.getBoundingClientRect().x;
      const breakpoint = this.dashboardUtils.getBreakpointInfoById(
        this.layouts.main.layoutCtx.breakpoint
      );
      ...
      if (breakpoint) {
        ...
        if (breakpoint.maxWidth) {
          maxWidth = isMobile
            ...
            : `${breakpoint.maxWidth - xOffset}px`;
        }
        if (breakpoint.minWidth) {
          minWidth = isMobile
            ? `${breakpoint.minWidth}px`
            : `${breakpoint.minWidth - xOffset}px`;
        }
      }
    ...
    return (
      prevMainLayoutWidth !== this.mainLayoutSize.width ||
      prevMainLayoutHeight !== this.mainLayoutSize.height ||
      prevMainLayoutMaxWidth !== this.mainLayoutSize.maxWidth ||
      prevMainLayoutMinWidth !== this.mainLayoutSize.minWidth
    );
  }
  ...
  private updateRightLayoutSize(): boolean {
    ...
    if (this.isEditingWidget && this.editingLayoutCtx.id === "right") {
      ...
      this.rightLayoutSize.width = this.isMobile
        ? "100%"
        : this.calculateWidth("right");
    }
    if (!this.isEditingWidget || this.editingLayoutCtx.id === "right") {
    ...
    return (
      prevRightLayoutWidth !== this.rightLayoutSize.width ||
      prevRightLayoutHeight !== this.rightLayoutSize.height
    );
  }
  ...
  private calculateWidth(layout: DashboardLayoutId): string {
    ...
    const mainLayout =
      this.dashboard.configuration.states[this.dashboardCtx.state].layouts.main;
    const rightLayout =
      this.dashboard.configuration.states[this.dashboardCtx.state].layouts
        .right;
    if (rightLayout) {
    ...
    if (layoutDimension) {
      if (layoutDimension.type === LayoutWidthType.PERCENTAGE) {
        ...
        const dashboardWidth =
          this.dashboardContainer.nativeElement.getBoundingClientRect().width;
  ...
  public openFilters($event: Event) {
    ...
    this.dialog
      .open<FiltersDialogComponent, FiltersDialogData, Filters>(
        FiltersDialogComponent,
        {
          disableClose: true,
          panelClass: ["tb-dialog", "tb-fullscreen-dialog"],
  ...
  public openDashboardSettings($event: Event) {
    ...
    const layoutKeys = this.dashboardUtils.isSingleLayoutDashboard(
      this.dashboard
    );
    if (layoutKeys) {
      const layouts = this.dashboardUtils.getDashboardLayoutConfig(
        this.dashboard.configuration.states[layoutKeys.state].layouts[
          layoutKeys.layout
        ],
    ...
    this.dialog
      ...
      .subscribe((data) => {
        if (data) {
          ...
          if (newGridSettings) {
            const layouts = deepClone(
              this.dashboard.configuration.states[layoutKeys.state].layouts
            );
            const layoutConfig = this.dashboardUtils.getDashboardLayoutConfig(
              layouts[layoutKeys.layout],
              this.layouts[layoutKeys.layout].layoutCtx.breakpoint
            );
            this.dashboardUtils.updateLayoutSettings(
              layoutConfig,
              newGridSettings
            );
    ...
    this.dialog
      ...
      >(ManageDashboardStatesDialogComponent, {
        ...
        data: {
          ...
          widgets: deepClone(this.dashboard.configuration.widgets) as {
            [id: string]: Widget;
          },
        },
      })
  ...
  private moveWidgets(
    ...
    this.cd.markForCheck();
    this.dialog
      .open<MoveWidgetsDialogComponent, any, MoveWidgetsDialogResult>(
        MoveWidgetsDialogComponent,
        {
          disableClose: true,
          panelClass: ["tb-dialog", "tb-fullscreen-dialog"],
      ...
      .subscribe((result) => {
        ...
        if (result) {
          const dashboardLayout =
            this.dashboardConfiguration.states[this.dashboardCtx.state].layouts[
              layoutId
            ];
          const targetLayout = this.dashboardUtils.getDashboardLayoutConfig(
            dashboardLayout,
            breakpointId
          );
          this.dashboardUtils.moveWidgets(
            targetLayout,
            result.cols,
            result.rows
          );
  ...
  private updateDashboardLayouts(newLayouts: DashboardStateLayouts) {
    this.dashboardUtils.setLayouts(
      this.dashboard,
      this.dashboardCtx.state,
      newLayouts
    );
    this.updateLayouts();
  }
  ...
  private updateStates(states: { [id: string]: DashboardState }) {
    ...
    if (!this.dashboard.configuration.states[targetState]) {
      targetState = this.dashboardUtils.getRootStateId(
        this.dashboardConfiguration.states
      );
    }
  ...
  public importWidget($event: Event) {
    ...
    this.importExport
      .importWidget(
        ...
        this.selectTargetLayout.bind(this),
        this.entityAliasesUpdated.bind(this),
        this.filtersUpdated.bind(this)
      )
  ...
  private editMissingAliases(
    ...
    customTitle: string,
    missingEntityAliases: EntityAliases
  ): Observable<EntityAliases> {
    return this.dialog
      .open<
        EntityAliasesDialogComponent,
        EntityAliasesDialogData,
    ...
    if (!this.widgetEditMode) {
      ...
      if (
        ...
        this.router.navigateByUrl(
          `customers/${this.currentCustomerId}/dashboards/${dashboardId}`
        );
      } else {
  ...
  public openDashboardState(state: string, openRightLayout?: boolean) {
    if (!this.destroyed) {
      const layoutsData = this.dashboardUtils.getStateLayoutsData(
        this.dashboard,
        state
      );
      if (layoutsData) {
  ...
  private updateLayouts(layoutsData?: DashboardLayoutsInfo) {
    if (!layoutsData) {
      layoutsData = this.dashboardUtils.getStateLayoutsData(
        this.dashboard,
        this.dashboardCtx.state
      );
    }
    for (const l of Object.keys(this.layouts)) {
      ...
      if (layoutsData[l]) {
        ...
        this.updateLayout(layout, {
          default: { widgetIds: [], widgetLayouts: {}, gridSettings: null },
        });
      }
    }
  }
  ...
  private updateLayout(
    ...
    layoutInfo: DashboardLayoutInfo
  ) {
    layout.layoutCtx.layoutData = layoutInfo;
    layout.layoutCtx.layoutDataChanged.next();
    this.dashboardUtils.updatedLayoutForBreakpoint(
      layout,
      this.isEdit ? layout.layoutCtx.breakpoint : this.dashboardCtx.breakpoint
    );
    this.updateLayoutSizes();
  }
  ...
  private entityAliasesUpdated() {
    this.dashboardCtx.aliasController.updateEntityAliases(
      this.dashboard.configuration.entityAliases
    );
  }

  private filtersUpdated() {
    this.dashboardCtx.aliasController.updateFilters(
  ...
  private notifyDashboardUpdated() {
    if (this.widgetEditMode) {
      ...
      this.dashboard.configuration.timewindow =
        this.dashboardCtx.dashboardTimewindow;
          ...
          catchError((err) => {
            if (err.status === HttpStatusCode.Conflict) {
              ...
              return this.dashboardService
                .getDashboard(this.dashboard.id.id)
                .pipe(
                  map((dashboard) =>
                    this.dashboardUtils.validateAndUpdateDashboard(dashboard)
                  )
                );
            }
  ...
  private isAddingToScadaLayout(): boolean {
    ...
    return layoutIds.every(
      (id) => layouts[id].gridSettings.layoutType === LayoutType.scada
    );
  }
  ...
  private selectTargetLayout(): Observable<DashboardLayoutId> {
    ...
    if (layoutIds.length > 1) {
      return this.dialog
        .open<SelectTargetLayoutDialogComponent, any, DashboardLayoutId>(
  ...
  addWidgetFromType(widget: WidgetInfo) {
    this.onAddWidgetClosed();
    this.widgetComponentService
      .getWidgetInfo(widget.typeFullFqn)
      .subscribe((widgetTypeInfo) => {
        const config: WidgetConfig =
          this.dashboardUtils.widgetConfigFromWidgetType(widgetTypeInfo);
        if (!config.title) {
        ...
        if (this.addingLayoutCtx?.breakpoint) {
          ...
          isDefaultBreakpoint =
            this.layouts.main.layoutCtx.breakpoint === "default";
        }
        const scada = this.isAddingToScadaLayout();
        if (scada) {
          newWidget = this.dashboardUtils.prepareWidgetForScadaLayout(
            newWidget,
            widgetTypeInfo.scada
          );
        }
        ...
        if (
          scada ||
          this.layouts.right.show ||
          !this.showLayoutConfigInEdit(this.layouts.main.layoutCtx)
        ) {
        ...
        if (widgetTypeInfo.typeParameters.useCustomDatasources) {
          ...
          this.dialog
            .open<AddWidgetDialogComponent, AddWidgetDialogData, Widget>(
              AddWidgetDialogComponent,
              {
                disableClose: true,
                panelClass: ["tb-dialog", "tb-fullscreen-dialog"],
  ...
  editWidget(
    ...
    if (this.editingWidgetOriginal === widget) {
      ...
      this.editingWidgetSubtitle =
        this.widgetComponentService.getInstantWidgetInfo(
          this.editingWidget
        ).widgetName;
  ...
  showLayoutConfigInEdit(layoutCtx: DashboardPageLayoutContext): boolean {
    return (
      layoutCtx?.gridSettings?.layoutType === LayoutType.divider ||
      (layoutCtx?.gridSettings?.layoutType === LayoutType.default &&
        (layoutCtx?.breakpoint === "default" ||
          (layoutCtx?.breakpoint !== "default" &&
            layoutCtx?.gridSettings?.viewFormat === ViewFormatType.list)))
    );
  }

  replaceReferenceWithWidgetCopy(
    $event: Event,
    layoutCtx: DashboardPageLayoutContext,
    widget: Widget
  ) {
    ...
    const widgetCopy = this.dashboardUtils.replaceReferenceWithWidgetCopy(
      widget,
      this.dashboard,
      this.dashboardCtx.state,
      layoutCtx.id,
  ...
  copyWidgetReference(
    ...
    this.itembuffer.copyWidgetReference(
      this.dashboard,
      this.dashboardCtx.state,
      layoutCtx.id,
      widget,
  ...
  removeWidget(
    ...
    if (!title || title.length === 0) {
      title =
        this.widgetComponentService.getInstantWidgetInfo(widget).widgetName;
    }
      ...
      .subscribe((res) => {
        if (res) {
          if (layoutCtx.widgets.removeWidgetId(widget.id)) {
            this.dashboardUtils.removeWidgetFromLayout(
              this.dashboard,
              this.dashboardCtx.state,
              layoutCtx.id,
              widget.id,
  ...
  exportWidget(
    ...
    $event.stopPropagation();
    this.importExport.exportWidget(
      this.dashboard,
      this.dashboardCtx.state,
      layoutCtx.id,
      widget,
  ...
  prepareDashboardContextMenu(
    layoutCtx: DashboardPageLayoutContext
  ): Array<DashboardContextMenuItem> {
    ...
    if (this.isEdit && !this.isEditingWidget && !this.widgetEditMode) {
      ...
      dashboardContextActions.push({
        ...
        enabled: this.itembuffer.canPasteWidgetReference(
          this.dashboard,
          this.dashboardCtx.state,
          layoutCtx.id,
          layoutCtx.breakpoint
  ...
  prepareWidgetContextMenu(
    ...
    if (this.isEdit && !this.isEditingWidget) {
      ...
      if (isReference) {
        widgetContextActions.push({
          action: (event, currentWidget) => {
            this.replaceReferenceWithWidgetCopy(
              event,
              layoutCtx,
              currentWidget
            );
  ...
  editWidgetsTypesToDisplay($event: Event) {
    ...
    const filterWidgetTypes =
      this.dashboardWidgetSelectComponent.filterWidgetTypes;
    const widgetTypesList = Array.from(
      this.dashboardWidgetSelectComponent.widgetTypes.values()
    ).map((type) => ({
      type,
      display:
        filterWidgetTypes === null ? true : filterWidgetTypes.includes(type),
    }));
    ...
    const providers: StaticProvider[] = [
      {
        ...
        useValue: {
          ...
          typesUpdated: (newTypes) => {
            this.filterWidgetTypes = newTypes
              .filter((type) => type.display)
              .map((type) => type.type);
            this.cd.markForCheck();
          },
  ...
  toggleVersionControl($event: Event, versionControlButton: MatButton) {
    ...
    if (this.popoverService.hasPopover(trigger)) {
      ...
      versionControlPopover.tbComponentRef.instance.popoverComponent =
        versionControlPopover;
      versionControlPopover.tbComponentRef.instance.versionRestored.subscribe(
        () => {
          this.dashboardService
            .getDashboard(this.currentDashboardId)
            .subscribe((dashboard) => {
              dashboard =
                this.dashboardUtils.validateAndUpdateDashboard(dashboard);
  ...
  get showMainLayoutFiller(): boolean {
    ...
    const dashboardMaxWidth =
      this.dashboardUtils.getBreakpointInfoById(this.dashboardCtx.breakpoint)
        ?.maxWidth || Infinity;
    return (
      !this.layouts.right.show &&
      layoutMaxWidth < dashboardMaxWidth &&
      !this.isEditingWidget
    );
  }

  get currentBreakpointValue(): string {
    return this.dashboardUtils.getBreakpointSizeDescription(
      this.layouts.main.layoutCtx.breakpoint
    );
  }

  private parseBreakpointsResponse(breakpoints: {
    [key: string]: boolean;
  }): BreakpointInfo {
    ...
    Object.keys(breakpoints).map((key) => {
      if (breakpoints[key]) {
        activeBreakpoints.push(
          this.dashboardUtils.getBreakpointInfoByValue(key)
        );
      }
    });
